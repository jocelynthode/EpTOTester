...
\end{figure}
As we increased the number of peers, our results showed that JGroups is not as scalable as EpTO, but considering our limited resources we couldn't fully observe the difference between JGroup and EpTO on this matter. Although we should bear in mind that in this experiment EpTO has a relatively large delay due to ordering of the events without a central node (as in JGroups). But despite of this delay, we expected that EpTO performs much better and our analysis is that due the insufficient resource on the testbed. Epto could not show its advantages.
\par
