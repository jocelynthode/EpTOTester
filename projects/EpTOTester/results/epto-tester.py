...
logging.info("Average time to deliver per peer in total: {:f} ms".format(average))
logging.info("Population std fo the time to deliver: {:f} ms" .format(statistics.pstdev(durations, average)))
logging.info("Average global time to deliver on all peers per experiment: {:f} ms".format(global_average))
...
for experiment_nb, the_stats in stats.items():
    ...
    logging.info("Experiment {:d}:".format(experiment_nb))
    logging.info("Total events sent: {:d}".format(sent_sum))
