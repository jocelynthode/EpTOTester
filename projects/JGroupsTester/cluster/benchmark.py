...
class Benchmark:
    ...
    def __init__(self, app_config, cluster_config, local, use_tracker, churn=None,
    ...
    def run(self, time_add, time_to_run, peer_number, runs=1):
        ...
        if self.local:
            ...
            service_image = self.app_config['repository'] + self.app_config['service']['name']
            for line in self.cli.pull(service_image, stream=True, decode=True):
                print(line)
            if self.use_tracker:
                tracker_image = self.app_config['repository'] + self.app_config['tracker']['name']
            ...
            try:
                ...
                ipam_pool = utils.create_ipam_pool(subnet=self.app_config['service']['network']['subnet'])
                ipam_config = utils.create_ipam_config(pool_configs=[ipam_pool])
                self.cli.create_network(self.app_config['service']['network']['name'], 'overlay', ipam=ipam_config)
            except errors.APIError:
                self.logger.info('Host is already part of a swarm')
                if not self.cli.networks(names=[self.app_config['service']['network']['name']]):
                    self.logger.error('Network  doesn\'t exist!')
                    exit(1)

        for run_nb, _ in enumerate(range(runs), 1):
            if self.use_tracker:
                self._create_service(self.app_config['tracker']['name'], tracker_image,
            ...
            self.logger.debug(datetime.utcfromtimestamp(time_to_start / 1000).isoformat())

            environment_vars = {**self.app_config['service']['parameters'],
            ...
            log_storage = self.cluster_config['local_data'] if self.local else self.cluster_config['cluster_data']
            self._create_service(self.app_config['service']['name'], service_image, env=environment_vars,
                                 mounts=[types.Mount(target='/data', source=log_storage, type='bind')],
                                 replicas=service_replicas, mem_limit=self.app_config['service']['mem_limit'])

            self.logger.info('Running Benchmark -> Experiment: {:d}/{:d}'.format(run_nb, runs))
            if self.churn:
                thread = threading.Thread(target=self._run_churn, args=[time_to_start + self.churn.delay], daemon=True)
                thread.start()
                self._wait_on_service(self.app_config['service']['name'], 0, inverse=True)
                ...
                if self.churn.synthetic:
                    ...
                    self._wait_on_service(self.app_config['service']['name'], containers_nb=total[0], total_nb=total[1])
                else:
            ...
            if not self.local:
                subprocess.call('parallel-ssh -t 0 -h hosts "mkdir -p {path}/test-{nb}/capture &&'
                                ...
                                .format(path=self.cluster_config['cluster_data'], nb=run_nb), shell=True)
                ...
                with open('hosts', 'r') as file:
                    for host in file.read().splitlines():
                        subprocess.call('rsync --remove-source-files '
                                        '-av {:s}:{:s}/*.txt ../data'
                                        .format(host, self.cluster_config['cluster_data']), shell=True)
                        subprocess.call('rsync --remove-source-files '
