...
(0..19).each { |i|
    ...
    ports.rotate!
    args = args.join(' ')
    puts "java -cp ../lib/neem-0.8-jar-with-dependencies.jar:src epto.utilities.App #{args} > localhost#{i}.txt 2>&1 &\n"
    `java -cp ../lib/neem-0.8-jar-with-dependencies.jar:src epto.utilities.App #{args} > localhost#{i}.txt 2>&1 &`
}
